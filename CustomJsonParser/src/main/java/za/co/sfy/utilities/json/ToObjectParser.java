...
public class ToObjectParser {
	...
	public Object parseStringToObject(String json, Class<?> jsonClass) {
		...
		HashMap<String, String> outerObjectMap = mapify(getOuterObject(json));
		Object objInst = null;
		...
		try {
			...
			for (Field field : objInst.getClass().getDeclaredFields()) {
				...
				case "long":
					field.setLong(objInst, Long.parseLong(outerObjectMap.get(field.getName())));
					
					break;
					
				case "class java.lang.String":
					field.set(objInst, (String) outerObjectMap.get(field.getName()));
					...
					if (json.contains(":[") && json != null) {
						...
						if (json.contains("},{") && json != null) {
							...
							for (int h = 0; h < (countMatches + 1); h++) {
								...
								setInnerListObject(newInstance, getListObjectObject(json, h), getListObject(json, h), json);
				...
				default:
					...
					HashMap<String, String> innerObjectMap = new ToObjectParser().mapify(innerObjP);
					
					setInnerObject(json, field, objInst, innerObjectMap);
	...
	protected Object setInnerListObject(Object newInstance, HashMap<String, String> mapify2,
		...
		for (Field field2 : newInstance.getClass().getDeclaredFields()) {
			...
			default:
				newListObjInst = setInnerObject(json, field2, newInstance, mapify2);
				break;
			}
		}
		
		return newListObjInst;
	}
